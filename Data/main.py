...
def total_cutpoints(vectors):		#use "global equal interval method" 
	...
	raw_value = vectors[:,:-1].astype(np.float)	#no decision column
	...
	cp_dict = {}		#every column's candidate cutpoints
	total_dict = []		#all column's candidate-cutpoint dict
	col_k_dict = {}		#col_order: k_interval_num
	for j in range(0, col_num):		#ignore decision column
		col_k_dict[j] = 2 	#initial col_k_dict dictionary
		column = raw_value[:,j]
	...
	while conflict:
		...
		col_k_dict[next_col] += 1 
		new_mat[:,nex_col] = cutpoint_again(raw_value[:,nex_col],
					total_dict[nex_col],col_k_dict[next_col])
		conflict, cf_set = check_conflict(new_mat,d_set)
	
	#---------------even only did cutpoint once, it needs to merge-----------#
